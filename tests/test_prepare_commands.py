...
from pheval_gado.prepare.prepare_commands import (
    CommandWriter,
    GADOPrioritiseCLIArguments,
    GADOProcessCLIArguments,
)
...
class TestCommandWriter(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.process_command_arguments = GADOProcessCLIArguments(
            input_output_dir=Path(self.test_dir),
        ...
        self.prioritise_command_arguments = GADOPrioritiseCLIArguments(
            input_output_dir=Path(self.test_dir),
            data_dir=Path("/data_dir"),
            gado_jar=Path("GADO/GADO.jar"),
            hpo_predictions=Path("genenetwork_bonf_spiked/genenetwork_bonf_spiked.dat"),
    ...
    def test_write_process_command(self):
        ...
        self.assertEqual(
            content,
            [
                "java -jar /data_dir/GADO/GADO.jar --mode PROCESS --output "
    ...
    def test_write_prioritise_command(self):
        ...
        self.assertEqual(
            content,
            [
                "java -jar /data_dir/GADO/GADO.jar --mode PRIORITIZE --output results_dir "
                "--caseHpoProcessed "
                f"{self.test_dir}/hpoProcessed.txt "
